/**
 * Physically-inspired displacement + specular maps for SVG feDisplacementMap,
 * ported from kube.io / winaviation liquid-glass-demo (Snell refraction on a convex bezel).
 */

export type ObscuraGlassSurface = "convex_squircle" | "convex_circle";

const SurfaceEquations = {
  convex_circle: (x: number) => Math.sqrt(1 - (1 - x) ** 2),
  convex_squircle: (x: number) => (1 - (1 - x) ** 4) ** 0.25,
} as const;
...
export type ObscuraLiquidGlassMaps = {
...
/** Must match lens element width/height for correct `backdrop-filter` sampling (kube.io). */
export const OBSCURA_LIQUID_GLASS_MAP_PX = 112;

const DEFAULT_MAP = OBSCURA_LIQUID_GLASS_MAP_PX;

/**
 * Build PNG data URLs + scale for feDisplacementMap (call from browser only).
 */
export function buildObscuraLiquidGlassMaps(options?: {
  mapSize?: number;
  surface?: ObscuraGlassSurface;
  bezelWidth?: number;
  glassThickness?: number;
  refractiveIndex?: number;
  refractionScale?: number;
}): ObscuraLiquidGlassMaps {
  const mapSize = options?.mapSize ?? DEFAULT_MAP;
  const surface = options?.surface ?? "convex_squircle";
  const bezelWidth = options?.bezelWidth ?? 26;
  const glassThickness = options?.glassThickness ?? 140;
  const refractiveIndex = options?.refractiveIndex ?? 1.5;
  const refractionScale = options?.refractionScale ?? 1.38;

  const surfaceFn = SurfaceEquations[surface];
  ...
  return {
    ...
    mapSize,
    displacementScale: maximumDisplacement * refractionScale,
  };
}

export function detectSvgBackdropFilterUrl(): boolean {
  if (typeof window === "undefined" || typeof document === "undefined") {
    return false;
  }
  const isChromium = Boolean((window as unknown as { chrome?: unknown }).chrome);
